...
class Home extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      emailID: '',
      partnerDID: processToken || undefined,
  ...
  async sendMessage() {
    if (this.state.partnerDID) {
      const token = await window.sdk.generateCredentialShareRequestToken()
      const mes = await window.messageService.send(this.state.partnerDID, { token})
      mes && alert('Request sent successfully')
    }
  }
  ...
  async sendEmailTo () {
    const { did, emailID } = this.state
    console.log('this.state: ', this.state);
      if (did&&emailID) { 
        sendEmail(did, emailID) 
      }
  }
  ...
  render() {
    ...
    return (
      <Fragment>
        {isAuthenticated && <MessageListener openResponseTable={(token)=>{this.setState({ResponseToken: token})}}/>}
        <div className='Home'>
          <form className='Form container'>
              ...
              <label>
                ...
                <div className='ShareToken'>
                  <input
                      ...
                      autoComplete="off"
                      name='partnerDID'
                      value={this.state.partnerDID}
                      onChange={(e) => this.setState({ partnerDID: e.target.value })}/>
                  <Button bsSize='large' onClick={(event) => this.sendMessage()}>Request Credential</Button>
                </div>
              </label>
            }
            ...
            { isAuthenticated && did &&
              <label>
                ...
                <div className='ShareToken'>
                  ...
                  <Button bsSize='large' onClick={(event) => this.sendEmailTo(event)}>Share</Button>
                </div>
              </label>
            }
            ...
            { isAuthenticated && haveCredentials &&
              <div className='Credentials'>
                ...
                <ResponseTable ResponseToken={this.state.ResponseToken}/>
              </div>
            }
            { isAuthenticated && haveCredentials &&
              <div className='Credentials'>
